# todo https://segmentfault.com/a/1190000008346992
import os
...
MNIST_DATA_PATH = os.path.expanduser('~/data/mnist')
...
# 开始训练和测试
W = tf.Variable(tf.truncated_normal([n_hidden, n_classes], tf.float32, stddev=0.1), dtype=tf.float32)
b = tf.Variable(tf.constant(0.1, shape=[n_classes]), dtype=tf.float32)
...
for i in range(2000):
    ...
    if (i + 1) % 200 == 0:
        ...
        print("Iter {}, step {}, train accuracy {}".format(
